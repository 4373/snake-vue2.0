  ...
  export default {
    ...
    methods: {
      ...
      turn(d) { // 转向
        if (!canTurn) return
        canTurn = false
        let b = this.direaction === 'ArrowLeft' && d === 'ArrowRight' || this.direaction === 'ArrowRight' && d ===
          'ArrowLeft' || this.direaction === 'ArrowUp' && d === 'ArrowDown' || this.direaction === 'ArrowDown' && d ===
          'ArrowUp'
        if (b) {
    ...
    watch: { // 暂停，开始
      ...
      long: function (n) { // 最高分
        let h = n - origin.length
        if (h > window.localStorage.getItem('high')) {
